...
| `hero-frame` | components | Destination template | ✅ | ✅ repoda. "Kadraj açılışı": containerlı dergi karesi pin'lenip fullbleed kapağa açılır (ters FLIP), scrim yükselir, başlık mürekkep aydınlanır. refreshPriority 10 (tabloda). LCP dostu — görsel hiç gizlenmez; reduced-motion/no-JS'te statik kadraj |
| `text-reveal` | animations | Çok sayfa | ❌ | ✅ repoda. Satır bazlı line-mask reveal preset'i (`data-text-reveal` + data-tr-stagger/duration/delay); bir kez oynar, bitince DOM orijinaline döner. `refreshPriority: -1`; pinli bölüm içinde kullanılmaz |
